...
public class AI_test : MonoBehaviour
{
    ...
    public float startAttackTime = 2; 
    ...
    private void Chasing()
    {
        ...
        if (!m_CaughtPlayer)
        {
            ...
            if(m_AttackTime <= 0 && Vector3.Distance(transform.position, GameObject.FindGameObjectWithTag("Player").transform.position) <= 1f){
                GameObject.FindGameObjectWithTag("Player").GetComponent<player_script>().changeHealth(-10);
                m_AttackTime = startAttackTime; // atc spd
            }
            else{
